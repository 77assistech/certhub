      ...
      <div onClick={e=>e.stopPropagation()} style={{ background:B.surface, border:`1px solid ${B.border}`, borderRadius:18, padding:"26px", width:"100%", maxWidth:460, boxShadow:"0 32px 80px rgba(0,0,0,0.5)", maxHeight:"90vh", overflowY:"auto" }}>
        ...
        {[{k:"client",l:"Nome completo",ph:"João da Silva"},{k:"document",l:"CPF / CNPJ",ph:"000.000.000-00"},{k:"email",l:"E-mail",ph:"joao@email.com"},{k:"phone",l:"Telefone",ph:"(11) 9 9999-9999"}].map(f=>(
          <div key={f.k} style={{ marginBottom:13 }}>
            ...
            <input placeholder={f.ph} value={form[f.k as keyof typeof form]} onChange={e=>setForm(frm=>({...frm,[f.k]:e.target.value}))} style={inp}/>
          </div>
        ))}
...
function MetricCard({label,value,sub,color,delay=0}:{label:string;value:string;sub?:string;color:string;delay?:number}) {
  const B_surface = "#1E1E1E";
  const B_border = "rgba(255,255,255,0.07)";
  const B_white = "#FFFFFF";
  const B_textSec = "#707070";
  return (
    <div style={{ background:B_surface, border:`1px solid ${B_border}`, borderRadius:14, padding:"18px 20px", position:"relative", overflow:"hidden", animation:`fadeIn 0.4s ease ${delay}ms both` }}>
      <div style={{ fontSize:11.5, fontWeight:600, color:B_textSec, marginBottom:10 }}>{label}</div>
      <div style={{ fontSize:24, fontWeight:700, color:B_white, letterSpacing:"-0.8px", lineHeight:1, fontFamily:"'Rajdhani',sans-serif" }}>{value}</div>
      {sub && <div style={{ fontSize:11.5, color:"#444", marginTop:4 }}>{sub}</div>}
      <div style={{ position:"absolute", bottom:0, left:0, right:0, height:2, background:`linear-gradient(90deg,${color}50,transparent)` }}></div>
    </div>
  );
}
